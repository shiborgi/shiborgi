    ...
    provider === 'anthropic'
      ...
      : {
          [provider]: {
            ...
            // 'placeholder' is the registry default: it works for OneCLI, which
            // MITMs HTTPS_PROXY and rewrites the Authorization header on the wire
            // regardless of what literal value the client sent. This install's
            // gateway is not a MITM proxy — the agent calls it directly by
            // baseURL — so the credential the client actually sends is the one
            // that identifies it. OPENAI_API_KEY carries that identity token when
            // a gateway provider injects one (see gatewaySessionEnv in
            // src/gateway-providers/gateway.ts); unset, this is byte-identical to
            // the registry default.
            options: { apiKey: process.env.OPENAI_API_KEY || 'placeholder', baseURL: proxyUrl },
