...
- [BREAKING] **Channel install skills are now the single source of truth — structured skill format.** Every `/add-<channel>` SKILL.md carries its mechanical steps as `nc:<kind>` directive fences (copy / append / dep / run / prompt / operator / env-set / json-merge) applied by a deterministic engine (`scripts/skill-apply.ts`); the setup wizard installs channels by driving the same SKILL.md (`setup/lib/skill-driver.ts` + `setup/channels/run-channel-skill.ts`), so wizard and skill can never drift. The prose remains the skill: an agent reading the SKILL.md with the fences stripped performs the same install, and anything the engine can't do degrades to an agent task. Authoring reference: [docs/skill-directives.md](docs/skill-directives.md); lint: `pnpm exec tsx scripts/skill-directives.ts <SKILL.md>`. **Migration:** the bespoke non-interactive channel installers (`setup/add-<channel>.sh`, `setup/install-<channel>.sh`) and the per-channel wizard flows (`setup/channels/<channel>.ts`) are deleted — anything that shelled out to them should apply the skill instead: interactively via the coding agent (`/add-<channel>`) or the setup wizard, or programmatically via `applySkill` with `inputs` / the `NC_INPUT_<VAR>` env convention (see [docs/skill-engine-seam.md](docs/skill-engine-seam.md) §6).
